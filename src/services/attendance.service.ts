...
import { AttendanceCheckType, AttendanceState, TimeResultType } from "../constants";
...
export class AttendanceService {
    ...
    private initUserAttendances(users: IUser[], userAttendances: IUserAttendances[], days: number) {
        let _users = [...users], _userAttendances = [...userAttendances];
        if (_userAttendances.length !== _users.length) {
            // to do 可优化
            _userAttendances = _users.map(user => {
                let _attendance = _userAttendances.find(x => x.name);
                return {
                    id: user.id,
                    name: user.name,
                    dept_name: user.dept_name,
                    attendances: _attendance ? _attendance.attendances : Array.from(new Array(days), () => [])
                }
            });
    ...
    */
    ...
    async generateUserAttendances(date, day = 8, name?: string) {
        ...
        for (let d of dates) {
            ...
            for (let user of users) {
                ...
                let currentIndex = userAttendances.findIndex(ul => ul.name === user.name);
    ...
    private async getLeaveTimeByMinutes() {
        const leaveType = [AttendanceState.C, AttendanceState.P, AttendanceState.S, AttendanceState.V];
        const leaveLog = this.attendances.filter(x => leaveType.includes(x.state));
    ...
    */
    ...
    private async getUserAttendanceLog() {
        ...
        let _attendance = { state: AttendanceState.O, value: 0 };
        let subTime = 0;
