import { IAttendanceLeaveResult, IAttendanceListResult, ICreateReport, IDepartmentIdResult, IDepartmentResult, IDingTalkDefaultBaseResult, IDingTalkTokenResponseResult, IDingTalkUserListIdResult, IDingTalkUserResult, IReportSimpleListResult, IUserAttendanceResult } from "@interfaces/dingTalk";
...
export default class DingTalkApi {
    ...
    async getDingTalkAccessToken() {
        const { access_token, expires } = global["dingTalkToken"] || { access_token: null, expires: 0 };
        ...
        if (!access_token || expires < currentUnix) {
            ...
            res.expires = moment().add(config.dingTalk.tokenExpires, "minutes").unix();
    ...
    async getDepartments(dept_id?: string) {
        ...
        const res = await fetch(`${this.v2ApiUrl}/department/listsub?access_token=${token}`,
    ...
    async getUserDetail(userId: string) {
        const token = await this.getDingTalkAccessToken();
        const res = await fetch(
            `${this.v2ApiUrl}/user/get?access_token=${token}`,
            {
                method: "POST",
                body: JSON.stringify({
                    userid: userId,
     ...
     * @param bodyData 
     */
    async createUserReport(bodyData: ICreateReport): Promise<IDingTalkDefaultBaseResult> {
        const token = await this.getDingTalkAccessToken();
        const res = await fetch(
