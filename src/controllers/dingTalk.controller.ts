...
export class DingTalkController {
    ...
    async createUser(@Body() createDto: IUserCreateDto) {
        ...
        let customAttendances = await FileData.readCustomAttendances(fileName);
        let userAttendances = await FileData.readAttendances(fileName);
        ...
        const result = await FileData.writeCustomAttendances(fileName, customAttendances);
        return result;
    }
